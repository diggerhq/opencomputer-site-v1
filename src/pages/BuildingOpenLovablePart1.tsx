    ...
    <div className="min-h-screen">
      ...
      <article className="max-w-[994px] mx-auto px-10 pt-10 pb-[60px]">
        ...
        <FadeIn delay={0.12}>
          <div className="space-y-7">
            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              This is a series about building a Lovable clone to learn how Lovable works
              under the hood. You can test the final result{" "}
              <a
        ...
        <FadeIn>
          <div className="space-y-7">
            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              A few years ago, in the early days of GPT, all we had were LLMs and APIs
              around them; not much of the infrastructure or patterns had been
              established. The way you would build something like Lovable was by
              scaffolding a lot of workflows around the LLM to make it happen. Lovable
              is a complex beast, but at the heart of it, we are generating a React app
              and previewing it. At the center of it, we have the user prompt, "I want
              to create a todo app." This would then get appended to a system prompt in
              order to start generating the code in a structured way, file by file,
              with the LLM writing it to the file system. Of course, our precious LLMs
              have limited context windows that we need to respect. So we can't just
              keep looping with an ever-growing context window. We need a lot of
              ...
              the context under control. Each subtask needs to have the right context
              injected and the ability to query it (yep, something like RAG to inject
              context). This is the part people now refer to as the agent loop, or the
              "harness."
            </p>

            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              So that's the first part: the agentic loop. As code gets generated, it
              also needs to be written to files so that we can generate the preview. To
              keep our backend secure, this code gets written to a remote, isolated
              container somewhere. The container can serve this code as a web service,
              and it is embedded into the application so that users can preview their
              creations. Yes, as you guessed, this is the part we now refer to as the
              sandbox.
            </p>
          </div>
        </FadeIn>
        ...
        <FadeIn>
          <div className="space-y-7">
            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              ...
              established at all. Everything was brand new, and people were figuring
              these concepts out from scratch. I don't think sandboxes even existed as a
              category back then. We also certainly did not have "harnesses" such as
              Claude Code at our disposal, so we had to create our own. Maybe we could
              add LangGraph, an amazing library that allowed us to assemble these
              agentic loops, workflows, and LLM tool calls to DIY a solution like the
              above.
            </p>

            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              As a result of how early we were, patterns emerged in a certain
              direction. We had the sandbox as an isolated concept, and we had the
              agent loop living somewhere else, doing its thing with the LLMs and
              eventually handling the code generation and writing to the remote sandbox.
            </p>

            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              Today we have ready-made harnesses that we can plug in to make our lives
              easier. While they come with their drawbacks, they are very good most of
              the time and can do the job. In many cases, you can get very far by
              reusing a harness such as the Agent SDK instead of rolling your own.
            </p>

            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              We have a similar story for sandboxes. Many sandboxes started as
              disposable places to run some code and then get destroyed quickly. "Run
              AI code" was the theme of these sandboxes.
            </p>

            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              As a result of how these two categories evolved, we traditionally ran the
              agent loop somewhere else and then had it communicate with a remote
              sandbox to run code, write files, and so on. This decision made sense
              back then, but I don't think it makes sense today. With sandboxes becoming
              more established, it makes sense to run our agent loop <em className="font-heading">inside</em> the sandbox.
              This gives us a lot of benefits: 1/ it is simpler to manage one
              component, 2/ there is less latency when it comes to reading and writing
              files.
            </p>
          </div>
        </FadeIn>
        ...
        <FadeIn>
          <div className="space-y-7">
            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              So the user just typed their prompt to create the generated app. What
        ...
        <FadeIn>
          <div className="space-y-7">
            <p className="text-[17px] leading-[1.75] tracking-[-0.1px]">
              ...
              and OpenComputer in this guide. We are able to retrieve and display the
              basic URL and have multi-turn conversations with the user. In the next
              parts, we will cover different aspects of making this basic prototype
              better:
            </p>
            ...
            <ol className="list-decimal list-inside space-y-3 text-[17px] leading-[1.75] tracking-[-0.1px] pl-2">
              ...
              <li>
                Dealing with more complex apps that have a backend, DB, and auth,
                generating them for users, and supporting integrations with other APIs
                (which is where the Lovable magic is)
              </li>
              <li>
                Customizing the harness and creating your own so that you can roll your
                own agent loops and have more control over how things work during code
                generation
              </li>
            </ol>
